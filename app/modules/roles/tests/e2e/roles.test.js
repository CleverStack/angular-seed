describe('e2e: roles', function() {
  ...
  beforeEach(function() {
    ...
    editButton = element(by.id('edit-role'));
    delButton  = element(by.id('delete-role'));
  ...
  it('Able to list roles and see the each column and buttons', function() {
    element(by.css('.col-head-id')).click();
    browser.driver.sleep(300);
    ...
    element.all(by.css('table tbody tr')).then(function(rows) {
      ...
      rows[0].all(by.css('td')).then(function(columns) {
        ...
        expect(element(by.css('.col-id .ng-binding')).getText()).not.toEqual(testRole.id);
      });
    });
  });
